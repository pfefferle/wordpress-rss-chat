...
Only posts with the built-in **chat** post format. A post is sent the first time it is saved while published, so a post that was skipped earlier (because its type was not enabled yet, for example) goes out on its next update. Once sent, later edits stay local; rss.chat keeps the first version. Posts in any other format are never sent.
